...
function getOffsetText(
  ...
  position: Position
): { start: number; end: number; cursor: number; text: string } {
...
export function getTag(
  ...
  const pureLine = offsetText.text
    .replace(/\{\{[^\}]*?\}\}/g, replacer("^"))
  ...
  pureLine.replace(
    /<([\-\w]+)(\s+[^>]*)?/g,
    (raw: string, name: string, attr: string, index: number) => {
      attrstr = offsetText.text.substring(index + raw.indexOf(attr));

      if (
        !tag &&
        index <= offsetText.cursor &&
        index + raw.length >= offsetText.cursor
      ) {
  ...
  if (tag != null && includeAttr) {
    (tag as Tag).attributes = getAttrs(attrstr);
  }
...
export function pureAttrName(name: string): {
  type: InputAttrType | undefined;
  name: string;
} {
  let type: InputAttrType | undefined;
  if (~name.indexOf("[(")) {
 ...
 */
export function typingPreChart(
  document: TextDocument,
  position: Position,
  count = 1
): string {
  return document.getText(
    new Range(new Position(position.line, position.character - count), position)
  );
}
