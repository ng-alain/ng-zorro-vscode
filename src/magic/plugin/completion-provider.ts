...
import {
  ...
  DirectiveTypeDefinitionComplex,
  InputAttrType,
  Tag,
} from "../interfaces";
...
export default class implements CompletionItemProvider {
  ...
  private genComponent(char: string): CompletionItem[] {
    return RESOURCES.filter((w) => w.type !== "pipe").map((i) =>
      this.renderCompletionItem(char, i, false)
    );
  }

  private genDirective(): CompletionItem[] {
    return RESOURCES.filter((w) => w.type === "directive").map((i) =>
      this.renderCompletionItem("", i, true)
    );
  }
  ...
  private genProperties(
    ...
    if (directives.length === 1) {
      return this.genPropertiesByDirective(
        document,
        position,
        triggerCharacter,
        tag,
    ...
    directives.forEach((directive) => {
      const list = this.genPropertiesByDirective(
        document,
        position,
        triggerCharacter,
        tag,
  ...
  private genPropertiesByDirective(
    ...
    if (directive.properties == null) return [];
    if (tag.isOnAttrValue) {
      ...
      if (attr.value.trim() === "") {
        ...
        if (property && property.typeDefinition) {
          ...
          let definitionList =
            property.typeDefinition as DirectiveTypeDefinition[];
          if (property.inputType === InputAttrType.Complex) {
            definitionList = this.getComplexDefinition(
              tag,
              definitionList as DirectiveTypeDefinitionComplex
            );
          }
        ...
        if (property && property.complexType) {
          return (
            directive.types?.[property.complexType].map((i) => {
    ...
    if (triggerCharacter === "[") {
      ...
      return directive.properties
        ...
        .filter(
          (w) =>
            w.inputType === InputAttrType.Input ||
            w.inputType === InputAttrType.InputOutput
        )
    ...
    if (triggerCharacter === "(") {
      ...
      return directive.properties
        ...
        .filter(
          (w) =>
            w.inputType === InputAttrType.Output ||
            w.inputType === InputAttrType.InputOutput
        )
  ...
  private renderAttrCompletionItem(
    tag: Tag,
    property: DirectiveProperty,
    ngBindingType: string,
    ...
    // 优化复杂类型
    if (property.inputType === InputAttrType.Complex) {
      const complexDefinitionList = this.getComplexDefinition(
        tag,
        property.typeDefinition as DirectiveTypeDefinitionComplex
      )
        .map((i) => i.value)
        .join(",");
      item.insertText = new SnippetString(
        `${property.name}="\${1|${complexDefinitionList}|}"$0`
      );
      return item;
    }
    let snippet = "";
    if (property.snippet) {
      ...
      switch (ngBindingType) {
        // 属性绑定
        case "property":
          snippet = `${property.name}]=${this.genDefaultPropertyValueSnippet(
            property
          )}`;
          break;
        // 事件绑定
        ...
        default:
          if (
            ...
            !["disabled", "loading"].some((key) =>
              property.pureName?.toLowerCase().includes(key)
            ) &&
    ...
    if (!snippet) {
      snippet = `${property.name}="\${1:${
        property.pureDefault ? property.pureDefault : property.pureName
      }}"$0`;
    }
  ...
  private genDefaultPropertySnippet(
    property: DirectiveProperty
  ): string | undefined {
    if (property.inputType === InputAttrType.InputOutput) {
      return `[(${property.name})]=${this.genDefaultPropertyValueSnippet(
        property
      )}`;
    }
    ...
    switch (property.type) {
      ...
      case "Enum":
        if (
          property.typeDefinitionSnippetStr &&
          property.typeDefinitionSnippetStr.length > 0
        ) {
          return `${property.name}=${this.genDefaultPropertyValueSnippet(
            property
          )}`;
    ...
    switch (property.type) {
      ...
      case "TemplateRef":
        return property.default
          ...
          : `"\${1:${property.pureName}}Tpl"$0`;
      case "Array":
      case "function":
        return `"\${1:${property.pureName}}"$0`;
      case "EventEmitter":
        if (property.typeRaw && ~property.typeRaw.indexOf("boolean")) {
  ...
  private getComplexDefinition(
    tag: Tag,
    definitionComplex: DirectiveTypeDefinitionComplex
  ): DirectiveTypeDefinition[] {
    const expectAttr = tag.attributes[definitionComplex.conditionField ?? ""];
    const expectValue = expectAttr ? expectAttr.value : "";
    const expectIndex =
      definitionComplex.list?.findIndex(
        (w) => w.conditionValue === expectValue
      ) ?? -1;
    return (
      definitionComplex.list?.[expectIndex < 0 ? 0 : expectIndex].values ?? []
    );
  }
