...
'nz-modal.whenToUse': localize('nz-modal.whenToUse', 'When requiring users to interact with the application without jumping to a new page to interrupt the user\'s workflow, you can use `Modal` to create a new floating layer over the current page for user-getting feedback or information purposes. Additionally, if you need to show a simple confirmation dialog, you can use `NzModalService.confirm()` and so on.'),
'nz-modal.description': localize('nz-modal.description', 'The dialog is currently divided into 2 modes, `normal mode` and `confirm box mode` (for instance, the `Confirm` dialog, which is called by `confirm/info/success/error/warning`). The degree of API support for the two modes is slightly different.'),
